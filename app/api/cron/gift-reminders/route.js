...
function emailHtml(profile, due) {
  const rows = due.map(({ reminder, days, recommendations }) => {
    ...
    const budget = reminder.budget_min || reminder.budget_max
      ...
      : "";
    return `<tr><td style="padding:18px 20px;border-bottom:1px solid #e9ecef"><strong style="font-size:17px;color:#111820">${escapeHtml(reminder.person_name)}</strong><br><span style="color:#59616a;line-height:1.6">${escapeHtml(reminder.occasion)} · ${escapeHtml(when)} · ${escapeHtml(timing)}${budget}</span></td><td style="padding:18px 20px;border-bottom:1px solid #e9ecef;text-align:right"><a href="${SITE_URL}/?q=${query}#shop" style="display:inline-block;background:#111820;color:#fff;text-decoration:none;border-radius:7px;padding:11px 15px;font-weight:700">See more gifts →</a></td></tr>${recommendationCards(reminder, recommendations || [])}`;
  }).join("");
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light only"></head><body style="margin:0;background:#f3f6f8;font-family:Arial,Helvetica,sans-serif;color:#111820"><div style="display:none;max-height:0;overflow:hidden;opacity:0">Your personalised GiftingGuru reminder and gift picks are ready.</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f6f8"><tr><td align="center" style="padding:28px 12px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:680px;background:#fff;border-radius:14px;overflow:hidden;border:1px solid #e5e8eb"><tr><td style="height:7px;background:linear-gradient(90deg,#ffd31a 0 50%,#55bceb 50% 100%)"></td></tr><tr><td align="center" style="padding:22px 24px 18px;background:#fff"><a href="${SITE_URL}" style="text-decoration:none"><img src="${SITE_URL}/giftingguru-logo.png" width="210" alt="GiftingGuru.co.za" style="display:block;width:210px;max-width:78%;height:auto;border:0"></a></td></tr><tr><td style="padding:26px 24px;background:#ffd31a"><p style="margin:0 0 7px;font-size:13px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#087eae">Never forget a special moment</p><h1 style="font-size:29px;line-height:1.12;margin:0 0 9px;color:#111820">A special day is coming up.</h1><p style="margin:0;font-size:16px;line-height:1.55;color:#27313a">Hi ${escapeHtml(profile.first_name)}, here are the dates and personalised gift ideas you asked us to remember.</p></td></tr><tr><td><table role="presentation" width="100%" cellspacing="0" cellpadding="0">${rows}</table></td></tr><tr><td align="center" style="padding:8px 24px 26px"><a href="${SITE_URL}/gift-reminders" style="display:inline-block;background:#087eae;color:#fff;text-decoration:none;border-radius:8px;padding:13px 20px;font-weight:700">Manage my gift reminders</a></td></tr><tr><td style="padding:22px 24px;background:#111820;color:#dce4ea;font-size:13px;line-height:1.6"><p style="margin:0 0 12px">Recommendations use the budget, interests and relationship you saved, together with current GiftingGuru stock. Prices and availability may change.</p><p style="margin:0 0 12px">This reminder service is free. You remain in control of your dates and email preferences.</p><p style="margin:0 0 12px"><a href="${SITE_URL}" style="color:#59bced">Shop GiftingGuru</a>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="${SITE_URL}/gift-reminders" style="color:#59bced">Manage reminders</a>&nbsp;&nbsp;·&nbsp;&nbsp;<a href="${SITE_URL}/api/reminders/unsubscribe?token=${escapeHtml(profile.unsubscribe_token)}" style="color:#59bced">Unsubscribe</a></p><p style="margin:0">GiftingGuru.co.za · South Africa<br><a href="mailto:hello@giftingguru.co.za" style="color:#fff">hello@giftingguru.co.za</a></p></td></tr></table></td></tr></table></body></html>`;
}
